...
connected [] n = []
connected (x:xs) n
  | fst x == n = snd x : connected xs n
  | snd x == n = fst x : connected xs n
...
connect i j [] = [(i,j)]
connect i j m
  | elem (i,j) m || elem (j,i) m = m
  | i < j                        = msort(m ++ [(i,j)])
  | otherwise                    = msort(m ++ [(j,i)])



disconnect :: Node -> Node -> Map -> Map
disconnect i j m = [ updatedMap | updatedMap <- m, updatedMap /= (i,j), updatedMap /= (j,i)]



add :: Party -> Event
add x (Game m n p ps) = Game m n (p ++ x) ps
add _ Over            = Over



addAt :: Node -> Party -> Event
addAt x y (Game m n p ps) = Game m n p updatedParties
  where
    updatedParties = take x ps ++ [y ++ (ps !! x)] ++ drop (x + 1) ps 
addAt _ _ Over            = Over
...
remove :: Party -> Event
remove players (Game m n p ps) = Game m n (removeSame players p) ps
  where
    removeSame players         = filter (`notElem` players) 
remove _ Over                  = Over



removeAt :: Node -> Party -> Event
removeAt x y (Game m n p ps) = Game m n p updatedParties
  where 
    updatedParties           = take x ps ++ [filter(`notElem` y) (ps !! x)] ++ drop (x + 1) ps
...
dialogue game (Action convo event) = do
  putStrLn convo 
  return (event game)
        


dialogue game (Branch condition true false) = do
  if condition game
    then dialogue game true
  else dialogue game false
      


dialogue game (Choice message choices) = do
  putStrLn message
  if null choices
    then return game
  else do
      display choices 1 -- displays dialogue choices using index from 1
      putStr ">> "
      userInput <- getLine


      if null userInput
        then dialogue game (Choice message choices)
      else if all isDigit (filter (`notElem` " ") userInput) -- considers cases for just numbers
        then do
          let chosen = read userInput :: Int
          if chosen == 0
            then return game  -- Entering 0 ends the dialogue
          else if chosen < 1 || chosen > length choices
            then do
                putStrLn ""
                putStrLn "Invalid choice, try again"
                putStrLn ""
                dialogue game (Choice message choices)
          else do
                let (_, dialogueNext) = choices !! (chosen - 1)  -- deals with the consequent dialogue
                dialogue game dialogueNext
      else do
        putStrLn ""
...
-- Function to display choices for dialogue
display :: [(String, Dialogue)] -> Int -> IO ()
display [] _                     = return ()
display ((choice, _):rest) index = do
  putStrLn (show index ++ ". " ++ choice)
  display rest (index + 1)



findDialogue :: Party -> Dialogue
findDialogue parties 
  | null dialogueParty         = Action "There is nothing we can do" id
  | otherwise                  = head dialogueParty
  where
    dialogueParty              = [snd chosenDialogue | chosenDialogue <- theDialogues, fst chosenDialogue == msort parties]
...
step (Game m n p ps) = do
  putStrLn ("You are in " ++ theDescriptions !! n)

  let locationOptions = retrieveLocations (connected m n) -- Gets a list of locations connected to current location
  let travelOptions = zip [1..] locationOptions -- Creates a list of pairs where first element is the index and the second element is locationOptions
  if null travelOptions
    then return()
  else do
    putStrLn "You can travel to:"
    let displayTravelOptions = zip [1..] (map snd travelOptions)
    printOptn displayTravelOptions

  let charactersInParty = zip [length travelOptions + 1 ..] p 
  if null charactersInParty
    then return ()
    else do
      putStrLn "With you are: "
      printOptn charactersInParty

  let visibleCharacters = zip [length travelOptions + length charactersInParty + 1 ..] (ps !! n)
  if null visibleCharacters
    then return ()
    else do
      putStrLn "You can see: "
      printOptn visibleCharacters
  ...
  userInput <- getLine
  if null userInput
    then return (Game m n p ps)

  else if all isDigit (filter (`notElem` " ") userInput) -- checks if user input is just numbers so that random inputs are taken care of
    then do
      let choices = map read  (words userInput) :: [Int]
      if (0 `elem` choices)
        then return Over
      else if all (\index -> index >= 1 && index <= length travelOptions) choices
          then do
            let promptOptn = locationOptions ++ p ++ (ps !! n)
            let userInputList = [snd x | x <- zip [1..] promptOptn, fst x `elem` choices]
            let locationIndex = head [ fst x | x <- zip [0..] theLocations, snd x `elem` userInputList]
            return (Game m locationIndex p ps) -- returns updated game with location
      else do
            let allCharacters    = charactersInParty ++ visibleCharacters
            let chosenCharacters = [char | i <- choices, (n, char) <- allCharacters, i == n]
            dialogue (Game m n p ps) (findDialogue chosenCharacters)
...
-- Function to display all the different choices
printOptn :: [(Int, String)] -> IO ()
printOptn = mapM_ (\(index, option) -> putStrLn (show index ++ ". " ++ option)) 

-- Function to get the required locations from theLocations
retrieveLocations :: [Int] -> [String]
retrieveLocations  = map (theLocations !!) 
...
gameLoop :: Game -> IO ()
gameLoop Over = putStrLn "Game Over!"
gameLoop game = do
  new <- step game
  gameLoop new

------------------------- Assignment 4: Safety upgrades
-- implemented when 0 inputted into Dialogue it returns original game state
-- when 0 inputted in the main game loop it exits the game back to prompt
-- in the main game inputs recognised are multiple numbers or single number or else it asks user to enter a valid input
-- entering nothing returns input prompt so that game does not crash
